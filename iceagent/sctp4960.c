...
static int  tcb_to_socket(sctp_tcb_t *tcb)
{
    if( tcb >= g_sctp_tcb && tcb < g_sctp_tcb + SCTP_SOCKET_MAX)
    {
        return (int)(tcb - g_sctp_tcb) + 1;
    }
    return -1;
}

// tcb->rx_buf, tcb->tx_buf untouched
static void sctp_reset_tcb(sctp_tcb_t *tcb)
{
    tcb->state = e_state_null;
...
int sctp_socket(void *parms)
{
    ...
    return  (j < SCTP_SOCKET_MAX ) ? (j + 1) : 0;
}
...
int sctp_bind(int sock, stcpaddr_t *addr, void *parms)
{
    ...
    if( sock < 0 || sock >= SCTP_SOCKET_MAX )
    {
        LOGV("[%s:%u] invalid socket\n", __func__, __LINE__);
        errno = EINVAL;
        e = -1;
        goto exit;
    }

    if( addr == 0 || addr->sin_port == 0 )
    {
        LOGV("[%s:%u] invalid address\n", __func__, __LINE__);
        errno = EINVAL;
        e = -1;
        goto exit;
    }

    sctp_tcb_t *tcb = g_sctp_tcb + sock;
    
    if( tcb->state != e_state_taken )
    {
        LOGV("[%s:%u] invalid state\n", __func__, __LINE__);
        errno = EINVAL;
        e = -1;
        goto exit;
    }
...
int sctp_connect(int sock, stcpaddr_t *addr, void *parms)
{
    ...
    if( tcb->state != e_state_taken && tcb->state != e_state_bound )
    {
        LOGV("[%s:%u] invalid state\n", __func__, __LINE__);
        errno = EINVAL;
        e = -1;
    ...
    if( tcb->state != e_state_bound )
    {
        tcb->self_addr.sin_port = addr->sin_port;
    }
...
int sctp_close(int sock, int reason)
{
    ...
    if( tcb->state == e_state_connected )
    {
        // initiate shutdown
    }

    tcb->state = e_state_shutdown;
...
static int sctp_make_data(void *tcb_ctx, void *buf, int len);

//
// send data type packet - in-thread call for now
// (1) prefix sctp_common_t as needed
// (2) prefix chunk header and TSN
//
...
//    pthread_mutex_lock(&tcb->lock);
    len = sctp_make_data(tcb, buf, len);
